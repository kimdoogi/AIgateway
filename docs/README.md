...
### 2026-08-24 — 플랫폼화: 운영 콘솔 v1 (발급 → 호출 → 정산을 화면으로)

- **`GET /console`** — 게이트웨이가 자체 서빙하는 운영 콘솔 (프레임워크·빌드체인 0, 단일 HTML, 전 렌더링 DOM API로 XSS 안전). 화면 6개: 시작하기(스니펫 자동 생성 — origin·키 주입) / 플레이그라운드(SSE 실시간·seq 표시·취소 D7) / 내 키·사용량(예산 바) / 가상 키 관리(발급·시크릿 1회 모달·비활성화) / BYO 키(등록·삭제 — 조회 API 없음이 설계) / 사용량 리포트(집계·CSV)
- **`GET /v0/usage` 신설** — 키 소유자 셀프서비스. 인증은 받되 쿼터·예산 게이트 면제: 402/429의 원인을 보는 창구가 같은 402/429로 막히면 순환이고, 대시보드 폴링이 실호출 쿼터를 소모하면 안 된다 (회귀 테스트로 고정)
- 검증은 **실제 브라우저 구동**으로 — 컨테이너 기동 → 관리 키 연결 → 발급 모달 → "플레이그라운드에서 사용" → claude 실호출 스트림(usage 28→65, $0.000353) → 내 사용량 바에 즉시 반영까지 화면에서 확인. 그 과정에서 `.env`의 localhost DATABASE_URL이 컨테이너 안에서 죽는 것을 `/ready` 503이 정확히 잡아내는 것도 관측(P0 프로브의 실전 첫 작동). 테스트 532개

### 2026-08-24 — 셀프 가입 포털: 관리자 없이 가입 → 발급 → 사용량

- **`GET /portal`** — 이메일 가입만으로 키를 받아 바로 쓰는 셀프서비스 포털. 계정=테넌트 1:1이라 키·사용량·BYO·원장이 계정 단위로 격리된다(타인 것은 404 — 존재 노출 금지). 마이그레이션 `0005_portal`(계정·세션), append-only 계약의 첫 실사용
- **공개 가입 = 남용 방어가 곧 설계**: 포털 발급 키는 항상 기본 한도(분당 rpm·기간 예산) 강제 — 계정이 스스로 못 올린다(상향은 관리자 소관). `PORTAL_INVITE_CODE` 초대 게이트, 로그인 브루트포스 이메일별 10/분, 미존재 계정도 같은 401·같은 scrypt 비용(존재 비노출). 비밀번호는 scrypt, 세션 토큰은 sha256 해시만 저장 — 가상 키와 같은 원칙
- 검증: 단위 12건(격리·한도·415 CSRF·브루트포스 429·발급 키의 실 게이트웨이 인증 통과) + **실제 브라우저 전 루프** — 가입 → 대시보드 → 발급 모달 → 그 키로 claude 실호출 2건($0.0002) → 예산 바·모델 집계 즉시 반영. `x-ratelimit-limit: 60` 헤더로 기본 한도 부착도 실측. 테스트 544개

### 2026-08-24 — Native IR 사용자 가이드

- [guides/native-ir.md](guides/native-ir.md) 신설 — 내부 스펙(ir-v0)이 아니라 **API 소비자 관점**의 실전 문서: 요청/응답 envelope 표, 블록 8종, 툴 루프 왕복 예시, 스트림 이벤트 카탈로그·재개·취소, **중간 모델 교체의 실제 사용법**("응답 message를 그대로 붙이고 model만 바꿔라"), 폴백 체인, providerOptions 네임스페이스 규칙(D5), effort 경계, 에러 category별 대응표, 자주 하는 실수. 포털·콘솔 스니펫이 보여주는 것의 "왜"를 채우는 문서
